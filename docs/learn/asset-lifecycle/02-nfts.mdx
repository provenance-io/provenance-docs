...
Single-asset NFTs are represented on Provenance Blockchain using the [Metadata module](/docs/sdk/metadata/).

The Metadata module was designed to work in conjunction with Provenance Blockchain’s [Client Execution Environment](/docs/pb/p8e/overview) (CEE), whose purpose is to control and reason about asset-specific data off-chain while using the blockchain for recording an immutable history of the asset data and metadata on-chain. This powerful capability provides data privacy for Personally Identifying Information (PII) and other Non-Public Information (NPI).
...
An NFT on Provenance Blockchain is defined by the [Scope](/docs/sdk/metadata/state#scopes) data structure.
